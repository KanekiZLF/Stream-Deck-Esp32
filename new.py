...
class Esp32DeckApp(ctk.CTk):
    ...
    def _apply_saved_led_colors_on_connect(self):
        """
        Aplica as cores dos LEDs salvas no JSON quando conecta.
        Só aplica se a cor for diferente do padrão (#FFFFFF) e se estiver conectado.
        """
        is_connected = self.serial_manager.is_connected or self.wifi_manager.is_connected
        if not is_connected:
            return
        
        self.logger.info("Aplicando cores dos LEDs salvas na configuração...")
        
        colors_to_apply = []
        
        # Primeiro, coleta todas as cores que precisam ser aplicadas
        for btn_key, btn_conf in self.config.data.get('buttons', {}).items():
            led_color = btn_conf.get('led_color', '#FFFFFF')
            
            # Só aplica se não for a cor padrão (branco)
            if led_color and led_color.upper() != '#FFFFFF':
                colors_to_apply.append((btn_key, led_color))
        
        # Aplica com delays crescentes para não sobrecarregar
        delay = 100  # ms
        for i, (btn_key, color) in enumerate(colors_to_apply):
            self.after(delay * i, lambda k=btn_key, c=color: self._send_led_color_command(k, c))
        
        if colors_to_apply:
            self.logger.info(f"Aplicadas {len(colors_to_apply)} cores de LEDs da configuração.")
        else:
            self.logger.info("Nenhuma cor personalizada encontrada na configuração.")

    def _reset_all_leds_on_disconnect(self):
        """
        Reseta todos os LEDs para branco quando desconecta.
        """
        self.logger.info("Resetando LEDs para padrão (branco) após desconexão...")
        
        # Envia comando para resetar todos os LEDs (se ainda estiver conectado)
        try:
            # Tenta enviar comando de reset via serial
            if self.serial_manager.is_connected:
                self.serial_manager.send_command("ALL_LED:OFF")
                time.sleep(0.1)
                self.serial_manager.send_command("ALL_LED:ON")
            # Tenta enviar comando de reset via Wi-Fi
            elif self.wifi_manager.is_connected:
                self.wifi_manager.send_command("ALL_LED:OFF")
                time.sleep(0.1)
                self.wifi_manager.send_command("ALL_LED:ON")
        except Exception as e:
            self.logger.error(f"Erro ao resetar LEDs na desconexão: {e}")
    ...
    def _set_header_visuals(self, connected: bool, connection_type: str = 'Serial'):
        ...
        if not connected:
            # Reset dos LEDs quando desconecta
            self._reset_all_leds_on_disconnect()
        else:
            # Aplica as cores salvas quando conecta
            self.after(500, self._apply_saved_led_colors_on_connect)  # Delay para garantir conexão estável
    ...
    def _send_led_color_command(self, button_key: str, color_hex: str):
        ...
        # Verifica se é uma cor válida e não é branco padrão
        if not color_hex or color_hex.upper() == '#FFFFFF':
            # Não envia comando para cor branca (padrão)
            return
    ...
    def _connect_any(self):
        ...
        if conn_type_logic == 'Serial':
            if self._connect_serial():
                # Aplica cores dos LEDs após conectar
                self.after(1000, self._apply_saved_led_colors_on_connect)  # Delay de 1 segundo
    ...
    def _disconnect_any(self):
        ...
        # Pequeno delay para garantir que o reset foi enviado
        time.sleep(0.2)
